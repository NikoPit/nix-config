...
- The user's software is almost entirely configured through Nix. When modifying a software configuration, modify the corresponding Nix configuration in `~/nix` instead of directly editing software configuration generated by NixOS or Home Manager.
- The Agent itself, including its skills, is also configured through Nix. When modifying the Agent's configuration or adding or modifying a skill, modify the corresponding skill configuration in the Nix configuration.
